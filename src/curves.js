...
Curve.prototype.closest=function(p, dsnap) {
    ...
    if (dsnap!=null) {
        ...
        if (dp0<dsnap)      { t=0; d=dp0; q=p0.clone(); }
        else if (dp2<dsnap) { t=1; d=dp2; q=p2.clone(); }
    }
...
Curve.prototype.intersectL=function(p, q) {
    return this.intersectV(p, p.to(q).normalized());
}


Curve.prototype.intersectV=function(p, v) {
    var u=p;
...
Curve.prototype.separate=function(p, v, L, R) {
    var tx=this.intersectV(p, v);
    var cs=[];
    
    if (tx.length==0) { cs.push(this); }
    else if (tx.length==1) {
        var t=tx[0];
        if (t==0 || t==1) { cs.push(this); }
        else { cs.extend(this.tsplit(t)); }        
    }
    else {
        tx.sort();
        var t0=tx[0], t1=tx[1];
