...
function dress() {
    ...
    draft.compose = function(size, msg) {
        ...
        //
        var scrop=(p67.x<p5.x);
        var sc65=p65.intersectLL(p67,p5,p57);
        var sc64=p64.intersectLL(p67,p5,p57);
        ...
        patH.slimmers=[MOVE,p64,p66,p65,(scrop?[sc65,MOVE,sc64]:p67),p64, MOVE,p61,p60,p62];  ////////
        patH.marks=[mhip,mhkd,mhszd];
        patH.midlines=[MOVE,p6,p53.ax(p6), MOVE,p9,p11, MOVE,p63b,p63.ax(p5), MOVE,p16h,p16h.ax(p1),p1,(bcrop?[]:[MOVE,p7,p55])];
        patH.guides=[MOVE,p1,p5,p8,MOVE,p4,p7,MOVE,p16,p15,p17,p10];
        patH.setMargin(size.margin);

        patEK.setBase([MOVE,p33h,p26,p43,cr1,p49,p31,CURVE,p31c,p30h,p33h]);
        patEK.midlines=[MOVE,p13,p14, (bcrop?[]:[MOVE,p45,p41]), MOVE,p43,p37];
        ...
        patES.midlines=[MOVE,p44,p52.ax(p44), MOVE,p51,p14b, (bcrop?[]:[MOVE,p54,p46]), MOVE,(bcrop?p38.ax(p5):p38),p12];
        ...
        var hipc=hcl;
        if (hipc.len()<uhipd) { uhipd-=hipc.len(); hipc=hch; }
        var uhipt=hipc.t(uhipd);
        
        var muhip=new Mark(hipc.tmid(uhipt).mirrored(up14,up17), hipc.tangent(uhipt).neg().mirroredv(up14,up17));
        var mueip=new Mark(kcum.p0, kcum.tangent(0).neg());
        var muvip=new Mark(up10, up10.to(up10d));
        


        var armP=new Pattern();

        armP.setBase([MOVE,upui,up13,CURVE,up13c,up10,CURVE,up10c,up14,hchm,hclm,uphk,upha,up18m,up17,up18,up16,upkam,upkkm,kcum.p2,CURVE,kcum.p1,kcum.p0]);
        armP.setMargin(size.margin);
