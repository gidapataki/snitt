        ...
        if (fetch==0) {
            ...
            if (typeof(cmd) == "string") {
                switch (cmd) {
                    ...
                    case TAG: fetch=2; break;
                }
            }
            else if (cmd instanceof Array)      { this.plot(ls[i]); }
            else if (cmd instanceof Polyline)   { dc.beginPoly(); this.plot(cmd.points); dc.endPoly(); }
            ...
            if (fetch==0) {
                switch (cmd) {
                    ...
                    case TAG: dc.pushTag(data[0]); this.plot((data[1] instanceof Array ? data[1] : [data[1]])); dc.popTag(); break;
...
PlotterContext.prototype.pushTag=function(tag)      {}
PlotterContext.prototype.popTag=function()          {}
PlotterContext.prototype.beginPoly=function()       {}
PlotterContext.prototype.endPoly=function()         {}
...
CanvasContext=inherit(PlotterContext, function(canvas, bg) {
    this.guides=true;
    this.canvas=canvas;
...
CanvasContext.prototype.drawInfo=function(assist) {
    ...
    if (hr!=null) { 
        ...
        var dx2=hr.pattern.len();
        if (dx<0) { dx+=hr.pattern.len(); }
        s+="   dx: "+dx.toFixed(2)+"   L-dx: "+(dx2-dx).toFixed(2);
    }
...
SVGContext=inherit(PlotterContext, function(svg) {
    this.svg=svg;
    this.dom=[svg];
    this.path=null;
    this.noTool=new NullTool();
...
SVGContext.prototype.selectTool=function(s) { 
    this.tool.leaveCtx(this);
    if (s in this.tools && (this.guides || s!="guide")) { this.tool=this.tools[s]; }
...
BoundingBoxContext=inherit(PlotterContext, function(bounds) { this.bounds=bounds; });
...
PatternContext=inherit(PlotterContext, function(pattern) {
...
PatternContext.prototype.beginPoly=function()               { this.poly=new Polyline(); }
PatternContext.prototype.endPoly=function()                 { this.pattern.addPolyline(this.poly); this.poly=null; }
PatternContext.prototype.moveTo=function(p)                 { if (this.poly) { this.poly.push(p); } this.current=p; }
PatternContext.prototype.lineTo=function(p)                 { var ep=p; if (this.poly) { this.poly.push(ep); } else { this.pattern.addLine(this.current, ep); } this.current=ep; }
PatternContext.prototype.curveTo=function(cp, p)            { var p1=cp, p2=p; this.pattern.addCurve(this.current, p1, p2); this.current=p2; }
PatternContext.prototype.pushTag=function(tag)              { this.tags.push(tag); this.pattern.startTag(this.tags.join(".")); }
PatternContext.prototype.popTag=function()                  { this.pattern.endTag(this.tags.join(".")); this.tags.pop(); }



// PolylineContext

PolylineContext=inherit(PlotterContext, function(ls, res) {
