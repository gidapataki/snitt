...
Line.prototype.closest=function(p, dsnap) {
    ...
    if (dsnap!=null) {
        ...
        if      (dp0<dsnap) { l=0; d=dp0; q=p0.clone(); }
        else if (dp1<dsnap) { l=1; d=dp1; q=p1.clone(); }
    }
...
Line.prototype.separate=function(p, v, L, R) {
    var p0=this.p0;
    var p1=this.p1;
    var q0=p.to(p0);
    var q1=p.to(q1);
    var s0=cross(v,q0).sgn();
    var s1=cross(v,q1).sgn();
    var sm=s0*s1;   // -1, 0, 1

    if (sm==1)      { (s0==1 ? L : R).push(this); }
    else if (sm==0) { (s0==-1 || s1==-1 ? R : L).push(this); }
    else {
        var u=p0.to(p1);
        var l=lambda(v,q0,u);
        var m=p0.mid(p1,l);
        (s0==1 ? L : R).push(new Line(p0,m));
        (s1==1 ? L : R).push(new Line(m,p1));
    }
}
